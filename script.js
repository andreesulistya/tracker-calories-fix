...
function renderBmrHistory() {
    ...
    [...riwayatFisik].sort((a,b) => b.tanggal.localeCompare(a.tanggal)).forEach((it) => {
        ...
        body.innerHTML += `
            <tr>
                <td><small>${formatTanggalIndo(it.tanggal)}</small></td>
                <td>${it.berat} kg</td>
                <td style="color:#007bff; font-weight:bold">${it.bmr}</td>
                <td style="color:#28a745; font-weight:bold">${it.tdee}</td>
                <td style="text-align:center;">
                    <div style="display: flex; justify-content: center; gap: 5px;">
                        <button class="btn-edit" onclick="bukaEditBmr(${realIdx})">
                            <span class="material-icons" style="font-size:18px;">edit</span>
                        </button>
                        <button class="btn-hapus" onclick="hapusBmr(${realIdx})">
